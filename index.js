	...
	client.channels.cache.get(channelId).messages.fetch(messageId).then(msg =>{
		...
		collector.on("collect", async i => {
			if(i.customId === "FichajeEntrar"){
				...
				if (i.guild.members.cache.get (user => user.id === "711522034345312308" || "928332789093388288")) {
					return interaction.reply({content: "Estas en la lista de pringados, ehh contacta con <@928332789093388288> y le preguntas, estar en la lista de pringados es por una razón no vas a estar por la cara, ve reflexiando amigo mio..", ephemeral: true})
				}
